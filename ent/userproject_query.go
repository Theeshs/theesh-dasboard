...
func (upq *UserProjectQuery) QueryUser() *UserQuery {
...
func (upq *UserProjectQuery) FirstID(ctx context.Context) (id uint, err error) {
	var ids []uint
...
func (upq *UserProjectQuery) FirstIDX(ctx context.Context) uint {
...
func (upq *UserProjectQuery) OnlyID(ctx context.Context) (id uint, err error) {
	var ids []uint
...
func (upq *UserProjectQuery) OnlyIDX(ctx context.Context) uint {
...
func (upq *UserProjectQuery) IDs(ctx context.Context) (ids []uint, err error) {
...
func (upq *UserProjectQuery) IDsX(ctx context.Context) []uint {
	ids, err := upq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (upq *UserProjectQuery) WithUser(opts ...func(*UserQuery)) *UserProjectQuery {
	query := (&UserClient{config: upq.config}).Query()
...
//		GroupBy(userproject.FieldProjectName).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.UserProject.Query().
//		Select(userproject.FieldProjectName).
//		Scan(ctx, &v)
...
func (upq *UserProjectQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*UserProject, init func(*UserProject), assign func(*UserProject, *User)) error {
	ids := make([]uint, 0, len(nodes))
	nodeids := make(map[uint][]*UserProject)
	for i := range nodes {
		fk := nodes[i].UserID
...
func (upq *UserProjectQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(userproject.Table, userproject.Columns, sqlgraph.NewFieldSpec(userproject.FieldID, field.TypeUint))
	_spec.From = upq.sql
