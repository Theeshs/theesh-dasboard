...
func (rq *RepositoryQuery) QueryUser() *UserQuery {
...
func (rq *RepositoryQuery) FirstID(ctx context.Context) (id uint, err error) {
	var ids []uint
...
func (rq *RepositoryQuery) FirstIDX(ctx context.Context) uint {
...
func (rq *RepositoryQuery) OnlyID(ctx context.Context) (id uint, err error) {
	var ids []uint
...
func (rq *RepositoryQuery) OnlyIDX(ctx context.Context) uint {
...
func (rq *RepositoryQuery) IDs(ctx context.Context) (ids []uint, err error) {
...
func (rq *RepositoryQuery) IDsX(ctx context.Context) []uint {
	ids, err := rq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (rq *RepositoryQuery) WithUser(opts ...func(*UserQuery)) *RepositoryQuery {
	query := (&UserClient{config: rq.config}).Query()
...
//		GroupBy(repository.FieldRepoName).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Repository.Query().
//		Select(repository.FieldRepoName).
//		Scan(ctx, &v)
...
func (rq *RepositoryQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Repository, init func(*Repository), assign func(*Repository, *User)) error {
	ids := make([]uint, 0, len(nodes))
	nodeids := make(map[uint][]*Repository)
	for i := range nodes {
		fk := nodes[i].UserID
...
func (rq *RepositoryQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(repository.Table, repository.Columns, sqlgraph.NewFieldSpec(repository.FieldID, field.TypeUint))
	_spec.From = rq.sql
	...
	if fields := rq.ctx.Fields; len(fields) > 0 {
		...
		if rq.withUser != nil {
			_spec.Node.AddColumnOnce(repository.FieldUserID)
		}
	}
